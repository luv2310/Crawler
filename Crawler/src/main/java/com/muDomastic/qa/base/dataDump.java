...
public class dataDump {
	...
	public void jsonVariables(String filePath)
	{
		...
		else if (filePath.toLowerCase().contains("go"))
		{
			source="goCompare";
		}
		else if (filePath.toLowerCase().contains("money"))
		{
			source="moneySuperMarket";
		}
		else if (filePath.toLowerCase().contains("linx"))
		{
			source="energyLinx";
		}else if (filePath.toLowerCase().contains("helpline"))
		{
		...
		try {
			...
			{    
				...
				action.postMethd(url,  firstJsonOnject.toString());
			}
		...
		{
			System.out.println(" :: Exception Occured in class dataDump Page, method name "+new Object(){}.getClass().getEnclosingMethod().getName()+" ::");
			e.printStackTrace();
		}
	}
	...
	public void setComparisonElements(LinkedHashMap ranks) 
	{
		try {
			if(ranks.containsKey("electricity")) 
			{
				...
				electricity_supplierName = ranks.get("supplierName").toString();
				...
				/**
				 * for some PCW We are not getting the night unit rate seprately so this is a hack in case we dont get night unit rate
				 * it should work fine
				 * */
				try
				{
					electricity_nightUnitRate = electricity.get("nightUnitRate").toString();  
				}
				catch (Exception e) {
			...
			if(ranks.containsKey("gas"))
			{
				...
				gas_supplierName = ranks.get("supplierName").toString();
